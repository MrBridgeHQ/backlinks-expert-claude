...
(للشفافية: أنا من يطوّر {{الموقع}}.) إن لم ترغب في بناء ذلك بنفسك، فهذه الأداة تقوم بالمهمة: {{funnel URL}} - لكن الطريقة اليدوية أعلاه تعمل بشكل جيد أيضاً.
```
...
```
The site serves merchant prices from an internal JSON endpoint, so a headless browser is overkill - grab the XHR call in DevTools and replay it with your region's query param for structured data.
